...
This project is a MERN stack application with three frontends. Follow these steps for a complete setup:

### 1. Prerequisites
- **Node.js**: v16+ recommended
- **MongoDB Atlas**: A free cluster (M0) is sufficient.

### 2. Fast Setup (Recommended)
Run the master setup script from the project root:
```powershell
./setup_all.ps1
```

### 3. Environment Configuration
Ensure `.env` files are present in the following locations (templates are provided):
...
CLIENT_URLS=http://localhost:3000,http://localhost:3001,http://localhost:5173,http://localhost:5174
```

#### Frontends (`user/.env`, `provider/.env`, `admin/.env`)
```env
...
- **User Port 3000**: `cd user && npm run dev`
- **Provider Port 5173**: `cd provider && npm run dev`
- **Admin Port 5174**: `cd admin && npm run dev`

### 5. Initial Data / Admin Approval
- Use `node backend/direct_approve.js` to quickly approve a provider account.
- Default Admin: Phone `9999999999`, Password `admin123` (if not created, register with role `admin`).
